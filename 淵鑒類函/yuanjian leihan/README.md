...
当前目录以数据文件为主，不包含完整应用入口代码；核心内容为按页切分的 `JSON` 分片，每个文件都保留了页面分类、正文摘录与词目分组信息，适合直接被脚本、服务端程序或检索系统读取。
...
root = Path(".")
data = json.loads((root / "0152.json").read_text(encoding="utf-8"))
...
const raw = fs.readFileSync("0152.json", "utf8");
...
- `groups` 更适合用于词目联想、典故提示与检索增强；如需严格校勘，请结合原始古籍再次核对。
